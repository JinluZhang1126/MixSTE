...
class  MixSTE(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
        ...
        self.STEblocks = nn.ModuleList([
            ...
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, vis=True)
    ...
    def ST_foward(self, x):
        ...
        for i in range(1, self.block_depth):
            ...
            x = tteblock(x)
            x = self.Temporal_norm(x)
            x = rearrange(x, 'b f (n cw) -> b f n cw', n=n)
...
from self_attention_cv import LinformerBlock, LinformerEncoder
...
class CrossAddFormer(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
                 num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,  norm_layer=None):
        """    ##########hybrid_backbone=None, representation_size=None,
    ...
    def forward(self, x):
        ...
        # now x shape is (b f (n cw))
        x = rearrange(x, 'b f (n cw) -> b f n cw', n=n)
        x = self.ST_foward(x, x_skip=x_t1)
...
from common.layers.edldense import DenseNormalGamma
class EDLFormer(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
                 num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,  norm_layer=None):
        """    ##########hybrid_backbone=None, representation_size=None,
        ...
        self.edlhead = DenseNormalGamma(n_input=3, n_out_tasks=3)
        # nn.init.kaiming_normal_(self.head[1].weight)
        # torch.nn.init.xavier_uniform_(self.head[1].weight)
        # torch.nn.init.normal_(self.head[1].bias, std = 1e-6)
    ...
    def forward(self, x):
        ...
        # EDL for all dimension
        x = self.edlhead(x)
        x = x.permute(1,0,2,3,4)    # For CUDA parallel
        # now x shape is [4,b,f,n,3]
        
        # # EDL for depth only
        # depth = x[:,:,:,-1].unsqueeze(-1)
        # edl_depth = self.edlhead(depth)
        # x = torch.cat((x[:,:,:,:2], edl_depth[0], edl_depth[1], edl_depth[2], edl_depth[3]), dim=-1)
        # # now x shape is [b,f,n,6]
        
        # pose, nu, alpha, beta = x[:,:,:,:3], x[:,:,:,3], x[:,:,:,4], x[:,:,:,5]
        # nu = F.softplus(nu).unsqueeze(-1)
        # alpha = F.softplus(alpha).unsqueeze(-1)
        # beta = F.softplus(beta).unsqueeze(-1)
        # x = torch.cat((pose, nu, alpha, beta), dim=-1)
        
        # edl = torch.abs(self.edlhead(x))
        
        # x = torch.cat((pose, edl), dim=-1)
