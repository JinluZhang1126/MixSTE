# Following the code implementation of Linear Multi-Head Attention
...
class LinearMultiheadAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, dropout=0.1, 
                 bias=True, qkv_bias=False, add_zero_attn=False, 
                 kdim=None, vdim=None, seq_len=243, proj_k=512, param_sharing='none',
                 qk_scale=None, attn_drop=0., proj_drop=0., comb=False, vis=False):
        super(LinearMultiheadAttention, self).__init__()
        embed_dim = dim
        proj_k = dim
        add_bias_kv = qkv_bias
        # self.qkv = nn.Linear(embed_dim, embed_dim * 3, bias=add_bias_kv)
    ...
    def forward(self, x, vis=False, key_padding_mask=None,
                need_weights=False, attn_mask=None):
        # type: (Tensor, bool, Optional[Tensor], bool, Optional[Tensor]) -> Tuple[Tensor, Optional[Tensor]]
        r"""
        Args:
            query, key, value: map a query and a set of key-value pairs to an output.
                See "Attention Is All You Need" for more details.
        ...
        """
        N, L, E = x.shape
        x = rearrange(x, 'N L E -> L N E')
        # - query: :math:`(L, N, E)` where L is the target sequence length, N is the batch size, E is the embedding dimension.
        # key and value is as same as query
        query, key, value = x, x, x   # make torchscript happy (cannot use tensor as tuple)
